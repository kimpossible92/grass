...
public class UIShopC : MonoBehaviour
{
    private Transform container;
    private Transform shopItemTemplate;
    [SerializeField] private MoneyMan money;
    public Button btn;
    ...
    void TaskWithParameters(long itemCost, Item item)
    {
        if (money.canBuyItems(itemCost))
        {
            ...
            FindObjectOfType<SoundMM>().Play("Money");

            if (item.Name.Contains("Seeds_Corn"))
            {
                GameM.instance.inventoryContainer.Add(item, 4);
            }
            else if (item.Name.Contains("Seeds_Tomato"))
            {
                GameM.instance.inventoryContainer.Add(item, 3);
            }
            else if (item.Name.Contains("Seeds_Strawberry"))
            {
                GameM.instance.inventoryContainer.Add(item, 6);
            }
            ...
            {
                GameM.instance.inventoryContainer.Add(item, 1);
            }
            else
            {
                GameM.instance.inventoryContainer.Add(item);
            }
        }
